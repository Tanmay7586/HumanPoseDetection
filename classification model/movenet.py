...
import cv2
from data import BodyPart, Person, person_from_keypoints_with_scores, compare_pose, CORRECT_POSES
import numpy as np
...
class Movenet(object):
    ...
    def _determine_crop_region(self, keypoints: np.ndarray, image_height: int,
        ...
        # Return the initial crop region if the torso isn't visible.
        else:
            return self.init_crop_region(image_height, image_width)
    ...
    def detect(self,
               ...
               reset_crop_region: bool = False,
               correct_pose_name: str = None) -> Dict:
        """Run detection on an input image and provide feedback.
        ...
        Args:
          ...
          reset_crop_region: Whether to use the crop region inferred from the
            ...
            value is True.
          correct_pose_name: Name of the correct yoga pose (e.g., "Tree Pose").

        Returns:
          A dictionary containing:
            - person: Detected keypoints and bounding box.
        ...
        person = person_from_keypoints_with_scores(keypoint_with_scores, image_height,
                                                   image_width)

        # Generate feedback if a correct pose is provided
        feedback = []
        if correct_pose_name and correct_pose_name in CORRECT_POSES:
            correct_pose = CORRECT_POSES[correct_pose_name]
            feedback = compare_pose(person.keypoints, correct_pose)
